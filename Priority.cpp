...
struct st {
	int id, at, bt, priority, cbt, ct = 0, fa = 1e9;
};
...
int main() {
	...
	int t_bt = 0;
	for (int i = 0; i < n; i++) {
		cin >> data[i].id >> data[i].at >> data[i].bt >> data[i].priority;
	...
	data[0].cbt--;
	while (cur != t_bt) {
		int mx_priotiry = 0, pos = -1;
		for (int i = 0; i < n; i++) {
			if (data[i].cbt != 0 && data[i].at <= cur && data[i].priority > mx_priotiry) {
				mx_priotiry = data[i].priority;
				pos = i;
			}
		}
		if (pos == -1) break;
